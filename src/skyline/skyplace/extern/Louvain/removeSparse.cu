 ...
 */
__global__ void computeEdgesSumSparse(device_structures deviceStructures) 
{
 ...
 */
__global__ void computeCommunityWeightSparse(device_structures deviceStructures) 
{
  ...
  //if(threadIdx.x == 0 && bestGain - vertexToCurrentCommunity[threadIdx.y] / M > 0) 
    if(threadIdx.x == 0 && communitySize[currentCommunity] < minSize)
      ...
      newVertexCommunity[vertex] = bestCommunity;
			printf("CommuniySize : %d currentCommunity: %d bestCommunitySize: %f bestCommunity: %d\n", 
...
__global__ void updateOriginalToCommunitySparse(device_structures deviceStructures) 
{
  int vertex = blockIdx.x * THREADS_PER_BLOCK + threadIdx.x;
